...
- Серверы для установки Grafana (Loki) для тестового и продуктового окружения
...
2. Перейти по пути **#root_folder#/infra/prod/ansible/** и ввести следующие команды: `ansible-playbook deploy_stage1.yml`.  
...
3. Перейти по пути **#root_folder#/infra/prod/ansible/PrometheusWithGrafana** и ввести следующие команды: `ansible-playbook deploy_monitoring.yml`. Данная команда развернет систему мониторинга на необходимом окружении. Будет установлен Prometheus, Grafana, node-exporters на все ВМ для снятия метрик, а так же cAdvisor для мониторинга приложения в контейнере. Из-за проблем с блокировкой доступа к некоторым зарубежным ресурсам, стоит поддерживать в актуальном состоянии установочный пакет Grafana в директории **#root_folder#/infra/prod(testing)/ansible/PrometheusWithGrafana/roles/grafana/files**
4. Перейти по пути **#root_folder#/infra/prod/ansible/Loki&Promtail** и ввести следующие команды: `ansible-playbook promtail-installation-playbook.yml` и после ее отработки `ansible-playbook loki-installation-playbook.yml` Данные сценарии развернут систему сбора и агрегирования логов на обоих окружениях. Используются Grafana Loki и в качестве агента Promtail.
5. На этом этапе на ВМ отведенной для раннера, устанавливается и регистрируется собственно сам раннер.  
    ...
    infra/
        prod/                   (директория для с необходимыми файлами для развертывания продуктового окружения)
            ansible/  
                ...
                Loki&Promtail/
                    resources/
                        loki-local-config.yaml      (файл конфигурации Loki)
                        loki.service                (юнит-файл сервиса Loki)
                        promtail-config-testing.yml (файл конфигурации Promtail для тестового окружения)
                        promtail-config.yml         (файл конфигурации Promtail для продуктового окружения)
                        promtail.service            (юнит-файл сервиса Promtail)
                    loki-installation-playbook.yml  (плейбук установки Loki)
                    promtail-installation-playbook.yml  (плейбук установки Promtail)
                    vars.yml                        (файл с переменными)
